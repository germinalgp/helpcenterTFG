...
(function($)
{
    ...
    function fileTemplate(list, fileobj, settings)
    {
	    ...
	    var upload	= $('<a title="Subir" class="ax-upload ax-button" />').click(function(){
	    	if(settings.enable) fileobj.sns();   
	    }).appendTo(toolbar).append('<span class="ax-upload-icon ax-icon"></span>');
	    
	    var remove	= $('<a title="Quitar" class="ax-remove ax-button" />').click(function(){
    ...
    var globalSettings = 
    {
		...
		selectText1:	'<b>Seleccionar ficheros</b>',
		selectText2:	'<b>Seleccionar ficheros</b>'
    };
	...
	var methods =
	{
		init : function(options)
		{
    	    return this.each(function() 
    	    {
				...
				var browse_c  	= $('<a class="ax-browse-c ax-button" title="A&ntilde;adir Ficheros" />').appendTo(fieldSet);//Browse container
				browse_c.append('<span class="ax-plus-icon ax-icon"></span> <span>A&ntilde;adir Ficheros</span>');
				
				var browseFiles = $('<input type="file" class="ax-browse" name="ax-files[]" />').attr('multiple',isAjaxUpload).appendTo(browse_c);
				
			    var uploadFiles = $('<a class="ax-upload-all ax-button" title="Subir ficheros seleccionados" />').appendTo(fieldSet);
			    uploadFiles.append('<span class="ax-upload-icon ax-icon"></span> <span>Subir ficheros seleccionados</span>');
			    
			    var removeFiles = $('<a class="ax-clear ax-button" title="Limpiar lista" />').appendTo(fieldSet);
			    removeFiles.append('<span class="ax-clear-icon ax-icon"></span> <span>Limpiar lista</span>');
